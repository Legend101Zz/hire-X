 ...
 */
export const apiPost = async (endpoint: string, data: Record<string, unknown>, token: string) => {
  const response = await apiCall(endpoint, {
    method: 'POST',
 ...
 */
export const apiPut = async (endpoint: string, data: Record<string, unknown>, token: string) => {
  const response = await apiCall(endpoint, {
    method: 'PUT',
    body: JSON.stringify(data),
    token,
